...
const server = http.createServer((request, response) => {
    ...
    switch (request.method) {
        ...
        case "DELETE":
            if (request.url.startsWith("/favoritos")) {

                const urlObj = new URL("http://localhost" + request.url);
                const nombreFav = urlObj.searchParams.get("nombre");

                let favoritos = JSON.parse(fs.readFileSync("./PRUEBA_02/json/tamas_favoritos.json").toString());

                let nuevaLista = [];
                for (let i = 0; i < favoritos.favoritos.length; i++) {
                    if (favoritos.favoritos[i].nombre != nombreFav) {
                        nuevaLista.push(favoritos.favoritos[i]);
                    }
                }

                favoritos.favoritos = nuevaLista;

                fs.writeFile("./PRUEBA_02/json/tamas_favoritos.json", JSON.stringify(favoritos, null, 2), err => {
                    if (err) {
                        response.statusCode = 500;
                        response.end(JSON.stringify({ mensaje: "Error al borrar de favoritos" }));
                    } else {
                        response.statusCode = 200;
                        response.setHeader("Content-Type", "application/json");
                        response.end(JSON.stringify({ mensaje: "Eliminado de favoritos" }));
                    }
                });

                return;
            }
            if (request.url.startsWith("/coleccion")) {
                ...
                let nuevaLista = [];
                for (let i = 0; i < coleccion.coleccion.length; i++) {
                    if (coleccion.coleccion[i].nombre != nombre) {
                        nuevaLista.push(coleccion.coleccion[i]);
                    }
                }

                coleccion.coleccion = nuevaLista;

                fs.writeFile("./PRUEBA_02/json/mis_tamagotchis.json", JSON.stringify(coleccion, null, 2), err => {
                    if (err) {
                        response.statusCode = 500;
                        response.end(JSON.stringify({ mensaje: "Error al borrar de colección" }));
                    } else {
                        response.statusCode = 200;
                        response.setHeader("Content-Type", "application/json");
                        response.end(JSON.stringify({ mensaje: "Eliminado de colección" }));
                    }
                });
